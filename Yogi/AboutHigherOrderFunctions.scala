...
class AboutHigherOrderFunctions extends KoanSuite {
  ...
  koan(
    ...
    makeUpper(List("abc", "xyz", "123")) should be("ABC", "XYZ", "123")

    makeWhatEverYouLike(List("ABC", "XYZ", "123"), {
      x => x.toLowerCase
    }) should be("abc", "xyz", "123")

    //using it inline
    List("Scala", "Erlang", "Clojure") map {_.length} should be(5, 6, 7)
  }
  ...
  koan("Currying allows you to create specialized version of generalized function") {
    ...
    customFilter(onlyEven)(xs) should be(12, 20, 2)

    val onlyEvenFilter = customFilter(onlyEven) _
    onlyEvenFilter(xs) should be(12, 20, 2)
